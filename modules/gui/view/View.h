...
class View {
public:
    View(Game *game, sf::Mutex &gameMutex) : screen(sf::VideoMode::getDesktopMode().width, sf::VideoMode::getDesktopMode().height),
        ...
        mapView.SetLeftTopPoint({0, 0});
        mapView.SetRightBottomPoint({window.getSize().x, window.getSize().y});
    }
